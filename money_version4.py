...
def add_n_day_period(times_series, ndays):
    ...
    for i, days in enumerate(days_mean.index):
        ...
        day_start = days.date()
        day_end = days.date() + timedelta(days=ndays-1)

        if day_end > times_series.iloc[-1].name.date():
            day_end = times_series.iloc[-1].name.date()


        bias[str(day_start):str(day_end)] = (bias[str(day_start):str(day_end)] -
                            days_mean.iloc[i]) / days_mean.iloc[i]

    # 添加周期调整值之后的 times_series
    new_times_series = (times_series / (1 + bias)).fillna(value=0, axis=0)
...
def dynamic_threshold(times_series, window):
    ...
    numerator[abs(numerator) < eps] = 0


    denumerator = times_series.rolling(window, min_periods=100).std()
    denumerator[abs(denumerator) < eps] = 0
...
def deal_reponse(response, window, ndays):
    ...
    for hullnum in response["aggregations"]["all_term_fields"]['buckets']:
        print(hullnum["key"])
        timestamps = []
...
def visual(result_dict):
    ...
    plt.title("SUM (period = {} days, rolling window = 100 points)".format(query["ndays"]))
    # plt.ylim(2.5,3)
